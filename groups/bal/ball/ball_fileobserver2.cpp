...
__out == true ==> EXISTS(logFilePattern, logFilePattern + strlen(logFilePattern), c, c == '%' && (c+1 != logFilePattern + strlen(logFilePattern)) && (*(c+1) == 'Y' || *(c+1) == 'M' || *(c+1) == 'D' || *(c+1) == 'h' || *(c+1) == 'm' || *(c+1) == 's' || *(c+1) == '%'))
...
(__out == 0 ==> bsl::ifstream(filename).is_open()) && (__out == -1 ==> !bsl::ifstream(filename).is_open())
