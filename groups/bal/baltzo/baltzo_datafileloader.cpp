...
(!loader.isRootPathPlausible() && !infile.is_open() ==> __out <= 0)
...
// CLASS METHODS
(!bdls::FilesystemUtil::isDirectory(path, true) ==> !__out) && (__out ==> bdls::FilesystemUtil::isRegularFile(u::concatenatePath(bsl::string(), path, "GMT").c_str(), true))
