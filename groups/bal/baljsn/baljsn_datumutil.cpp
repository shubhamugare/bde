...
/// `*tokenizer`, updating the specified `*errorStream` if any errors are
/// detected, including if the specified `maxNestedDepth` is exceeded.
(maxNestedDepth < 0 ==> __out == -4) && (tokenizer->tokenType() == baljsn::Tokenizer::e_ERROR ==> __out == -1) && (baljsn::Tokenizer::e_ELEMENT_NAME != tokenizer->tokenType() ==> __out == -2) && ((0 != baljsn::ParserUtil::getUnquotedString(&key, tokenContents)) || (0 != rc) ==> __out == -3
...
/// `*tokenizer`, updating the specified `*errorStream` if any errors are
/// detected, including if the specified `maxNestedDepth` is exceeded.
(maxNestedDepth < 0 ==> __out == -4) && (tokenizer->tokenType() == baljsn::Tokenizer::e_ERROR ==> __out == -1) && (decodeValue(..., &elementValue, ...) != 0 ==> __out == -2) && (__out == 0 ==> tokenizer->tokenType() == baljsn::Tokenizer::e_END_ARRAY)
...
(__out == 0) || (__out != 0 && __out == u::encodeValue(formatter, datum[i], strictTypesCheckStatus) for some i < datum.length())
