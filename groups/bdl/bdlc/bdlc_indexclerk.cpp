...
// PRIVATE ACCESSORS
(!__out ==> (indicesInvalid || (indicesNotUnique & 0x2))) && (__out ==> !(indicesInvalid || (indicesNotUnique & 0x2)))
bool
...
// ACCESSORS
(std::find(d_unusedStack.begin(), d_unusedStack.end(), index) != d_unusedStack.end() ==> !__out) && (std::find(d_unusedStack.begin(), d_unusedStack.end(), index) == d_unusedStack.end() ==> __out)
