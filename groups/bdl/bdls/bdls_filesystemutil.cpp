...
__out.data() == str.data() + idx || (len == bsl::string::npos && __out.size() + idx == str.size())
...
__out == (*path == '.' && (path[1] == '\0' || (path[1] == '.' && path[2] == '\0')))
static inline
...
__out == true ==> (path != nullptr && (std::string(path) == "." || std::string(path) == ".." || std::string(path).substr(std::string(path).size()-2) == "/." || std::string(path).substr(std::string(path).size()-3) == "/.."))
static inline
