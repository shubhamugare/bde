...
/// casts that one is never accidentally casting between pointers to `char`
/// or `OctetType` and pointers to `unsigned int`.
__out == reinterpret_cast<unsigned char*>(ptr)
static inline
...
/// otherwise.
__out == ((oct & k_CONTINUE_MASK) == k_CONTINUE_TAG)
static inline
...
__out == true ==> uc < (1 << (k_TWO_OCT_CONT_WID + k_CONTINUE_CONT_WID))
static inline
...
/// UTF-8 octets or less and `false` otherwise.
__out == (0 == (uc & (~ (unsigned int) 0 << (k_FOUR_OCT_CONT_WID + 3 * k_CONTINUE_CONT_WID))))
static inline
