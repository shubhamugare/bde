...
static const bsls::Types::Uint64 CRC_TABLE[256] = {
    0,
...
bsl::ostream& Crc64::print(bsl::ostream& stream) const
{
    ...
    bsls::Types::Uint64 crc = checksum();

    char out[] = "0x0123456789abcdef";  // room for 16 digits plus "0x" and NUL
    for (int i = 0; i < 8; ++i) {
        unsigned char byte = (crc >> (56 - (i * 8))) & 0xff;
        out[2 + 2 * i] = hex[byte >> 4];
        out[3 + 2 * i] = hex[byte & 0xf];
    }
    return stream << out;
}
