...
(__out == 0 && *cursor + bsls::AlignmentUtil::calculateAlignmentOffset(&buffer[*cursor], alignment) + size > bufSize) || (__out != 0 && __out >= &buffer[*cursor] && __out < &buffer[*cursor] + bufSize && (reinterpret_cast<intptr_t>(__out) % alignment) == 0)
static
...
// CLASS METHODS
(size <= 0 ==> __out == nullptr) && (size > 0 ==> __out != nullptr)
